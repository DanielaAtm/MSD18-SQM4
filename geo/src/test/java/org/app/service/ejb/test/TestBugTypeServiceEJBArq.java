...
public class TestBugTypeServiceEJBArq {
	...
	public void test4_GetBugTypes() {
		...
		Collection<BugType> bugTypes = service.toCollection();
		assertTrue("Fail to read bug types!", bugTypes.size() > 0);
	}
	@Test
	public void test3_AddBugTypes() {
		...
		for (int i=1; i <= bugTypeToAdd; i++){
			service.add(new BugType(i, "BugType_" + (100 + i),null));
		}
		Collection<BugType> bugTypes = service.toCollection();
		assertTrue("Fail to add Employees!", bugTypes.size() == bugTypeToAdd);
	}
	...
	public void test2_DeleteBugType() {
		...
		for (BugType bt: bugTypes)
			service.remove(bt);
		Collection<BugType> bugTypesAfterDelete = service.toCollection();
		assertTrue("Fail to read bug types!", bugTypesAfterDelete.size() == 0);
	}	
}
